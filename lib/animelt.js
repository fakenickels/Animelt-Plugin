(function($, undefined) {
  ...
  var rCssValue = /([#0-9.]+)([#a-z%]+)?/gi,
    isHexColor = /#[a-f0-9]/gi,
    digits = /[0-9.]+/g,
    dTest = /[0-9]/g,
    specials = {},
    quickJ = $([1]);
  ...
  var animelt = ($.animelt = {
    ...
    fx: {
      updateComplexStr: function(startValue, finalValue, p) {
        ...
        if (!$.isArray(startValue)) old = startValue.match(digits) || 0;
        else old = startValue;
        ...
        return finalValue.replace(rCssValue, function(exp, num, unit) {
          ...
          var curValue =
            Number(old[i] || 0) + (Number(num) - Number(old[i] || 0)) * p;
          i++;
      ...
      updateABunchOfProps: function(props, p) {
        ...
        $.each(props, function(prop, vals) {
          updatedProps[prop] = animelt.fx.updateComplexStr(
            vals.origin,
            vals.final,
            p
          );
